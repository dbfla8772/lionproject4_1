...
def createPost(request):     #new.html의 정보를 받아서 데이터베이스에 저장
    ...
    new_post.pub_date = timezone.now()      #현재시각을 pub_date 컬럼에 넣어줌
    new_post.save()
    return redirect('detailHtml', new_post.id)      # 기능을 사용하고 원래 페이지로 돌아가야함. redirect 사용, 새로운 객체의 id를 보내줘야함.
...
def upadatePost(request, id):   #id 값을 받음. 기존의 수정해야 할 id 값을 받음. edit.html에서 수정한 내용을 데이터베이스에 적용
    ...
    return redirect('detailHtml', update_post.id)      
